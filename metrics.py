...
def compute_img_metric(im1t: torch.Tensor, im2t: torch.Tensor,
    ...
    """
    im1t, im2t: torch.tensors with batched image shape, range from (0, 1)
    """
    ...
    for i in range(batchsz):
        if metric in ["mse", "psnr"]:
            ...
            # Adjust win_size here and include the data_range argument
            value, ssimmap = photometric["ssim"](
                im1[i], im2[i], 
                multichannel=True, 
                full=True, 
                win_size=min(hei, wid, 3),  # set a custom win_size
                data_range=1.0  # Specify the data_range explicitly
            )
            if mask is not None:
